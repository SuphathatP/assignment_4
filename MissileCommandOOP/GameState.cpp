...
void GameState::NewGame()
{
	...
	base_maxMissiles.clear(); // base_maxMissiles
...
void GameState::Update(float elapsedTime)
{
	...
	for (size_t b = 0; b < base_pos.size(); b++)
	{
		...
		if (base_missileCount[b] < base_maxMissiles[b]) // base_maxMissiles
		{
			base_timeSinceReload[b] += elapsedTime;
			if (base_timeSinceReload[b] >= BASE_RELOAD_TIME)
			{
				base_missileCount[b]++;
	...
	// Input
	if (Play::Input::GetMouseDown(Play::Input::MouseButton::BUTTON_LEFT) && !mouseFired || Play::Input::KeyPress(Play::KEY_E) && !mouseFired)
	{
		mouseFired = true;
	...
	for (size_t i = 0; i < explosion_pos.size(); )
	{
		...
		float lifeFraction = 1.0f - (explosion_life[i] / EXPLOSION_MAX_LIFE);
		explosion_radius[i] = EXPLOSION_MAX_RADIUS * lifeFraction;

		// Apply damage to nearby cities and bases.
		if (explosion_life[i] < EXPLOSION_MAX_LIFE && explosion_life[i] + elapsedTime >= EXPLOSION_MAX_LIFE)
		{
	...
	for (size_t i = 0; i < missile_origin.size(); i++)
	{
		...
		Play::DrawLine(missile_origin[i], endPoint, missile_color[i]);
		Play::DrawPixel(endPoint, Play::cWhite);
		Play::DrawCircle(missile_target[i], 2, missile_color[i]);
	}
